...
def _map_stock_to_industry(ticker: Optional[str]) -> Optional[str]:
    """根据股票代码映射到核心行业（硬编码，支持 5 个核心行业）。

    Args:
        ticker: 股票代码，如 "000725.SZ", "688981.SH", "601857.SH", "600036.SH" 等

    Returns:
        匹配到的核心行业名称（如 "消费电子", "新能源车", "半导体", "石油化工", "金融地产"），未配置或不识别则返回 None
    """
    ...
    # 硬编码映射表 (支持 5 个行业: 消费电子、新能源车、半导体、石油化工、金融地产)
