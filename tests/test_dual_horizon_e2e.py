...
Assertions follow the post-DAV-8 contract: the dual-horizon result carries
aggregated top-level ``not_applicable`` (any completed horizon) and
...
def test_dual_horizon_propagates_both_horizons_to_result_sse_and_reportdb():
    ...
    # DAV-8 contract: medium horizon is not_applicable, so the dual-horizon
    # result aggregates not_applicable=True and the deduped falsification list.
