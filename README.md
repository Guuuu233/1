...
分析结果结构化存储，支持按标的、日期检索历史研报，决策卡片一目了然地展示方向、置信度、目标价与止损价。中文本地化之前生成的历史英文报告会自动标注「旧版报告」标识，并在详情页提示正文可能为英文。
...
系统内置多数据源自动降级链，A 股数据以 AKShare（东财/新浪）为主，并支持以下独立数据源：

| 数据源 | 覆盖 | 配置 |
|--------|------|------|
| AKShare（东财/新浪） | 行情、技术指标、新闻、资金流、涨跌停、龙虎榜等 | 免费源，默认启用 |
| BaoStock | 行情、财务（备用） | 免费源 |
| Investoday 今日投资 | 行情、前复权 K、财报、新闻、高管持股 | `INVESTODAY_API_KEY` |
| **同花顺（fuyao.aicubes.cn）** | 财务数据（三大报表+财务指标）主源；涨跌停池/龙虎榜备用源；历史 K 线末位备用（排第 5，yfinance 之后）；行情快照第 3 备用；交易日历在线对照 | `FUYAO_API_KEY`（到 https://fuyao.aicubes.cn/ 登录签发） |

**同花顺接入说明**

- 财务数据（利润表/资产负债表/现金流/财务指标）默认走 `cn_fuyao` 主源，失败降级到现有弱源；
- 涨跌停池与龙虎榜以东财为主，`cn_fuyao` 在东财失败时顶上；
- 行情快照（`realtime_data`）链为东财→新浪→`cn_fuyao`（第三备用）；历史 K 线（`core_stock_apis`）
  链为 `cn_akshare→cn_baostock→cn_investoday→yfinance→cn_fuyao`（`cn_fuyao` 排第 5，在
  yfinance 之后，仅作末位兜底）；
- 交易日历在 AKShare 不可用时用同花顺近一年交易日作在线对照/备用；
- API Key 严禁写入代码或提交；配置到 `.env` 的 `FUYAO_API_KEY` 即可。
