...
> **启动守卫**：`TA_APP_SECRET_KEY` 未设置时，API 服务与调度器都会**拒绝启动**（防止使用内置默认密钥导致加密/JWT 被破解）。请在启动前设置，例如 `export TA_APP_SECRET_KEY=$(openssl rand -base64 32)`。仅限本地开发：可显式设置 `TA_ALLOW_DEFAULT_SECRET=1` 放行内置默认密钥（不安全，禁止用于生产）。
