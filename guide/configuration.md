...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| **`TA_APP_SECRET_KEY`** | （空，启动守卫拒绝启动） | 加密用户 LLM API Key、签发登录 JWT 的主密钥。**未设置时服务（API 与调度器）拒绝启动**。生产环境必须设置，生成：`openssl rand -base64 32`。首次设置会自动把已有数据从默认密钥迁移到新密钥；设置后不可更改 |
| `TA_ALLOW_DEFAULT_SECRET` | （空） | **仅限本地开发**：设为 `1` 放行内置默认密钥（不安全，禁止用于生产）。仅当确需在未设置 `TA_APP_SECRET_KEY` 时本地调试才使用 |
