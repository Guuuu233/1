...
Force an isolated SQLite database before any test imports api.database /
api.main so tests cannot touch a real checkout database, and initialize the
schema at session start (the FastAPI lifespan only runs when TestClient is
used as a context manager, which most tests do not do).
"""
